      ...
      <table className="w-full min-w-[680px] border-collapse">
        <thead>
          <tr className="border-b border-line bg-raised">
            <th className="w-14 px-2 py-2.5 text-xs font-semibold uppercase tracking-wide text-muted">
              Soat
            </th>
        ...
        <tbody>
          {Array.from({ length: P }, (_, p) => (
            <tr key={p} className="border-b border-line-soft last:border-0">
              <td className="bg-raised/60 px-2 py-1.5 text-center text-xs font-semibold text-muted">
                {p + 1}
              </td>
              {Array.from({ length: D }, (_, d) => {
                if (kind === 'class') {
                      ...
                      >
                        {cell.parts.map((pt, i) => (
                          <div key={i} className={i > 0 ? 'mt-1 border-t border-line-soft pt-1' : ''}>
                            ...
                            {showTeacher && (
                              <div className="truncate text-[11px] text-muted">{pt.teacherName}</div>
                            )}
                            {pt.week !== 'all' && (
